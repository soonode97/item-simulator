...
router.post('/inventories/purchase/:charactersId', authMiddleware, async (req, res, next) => {
    ...
    if (!inventory) {
        return res.status(400).json({ errorMessage: '인벤토리가 존재하지 않습니다.' });
    }
    ...
    if (characterInfo.money < item.item_price) {
        return res.status(500).json({ errorMessage: '소지하고 있는 골드가 부족합니다.' });
    }
...
router.post('/inventories/sell/:charactersId', authMiddleware, async (req, res, next) => {
    ...
    if (!inventoryItem) {
        return res.status(400).json({ errorMessage: '인벤토리에 판매할 아이템이 존재하지 않습니다.' });
    }
    ...
    return res.status(201).json({ message: '아이템이 판매되었습니다.', '보유 잔액': characterInfo.money + sellPrice });
});
...
router.get('/inventories/:charactersId', authMiddleware, async (req, res, next) => {
    ...
    if (character.accountsId !== +accountsId) {
        return res.status(401).json({ errorMessage: '계정 권한이 없어 조회가 불가합니다.' });
    }
